...
// Load Validation for expereince
...
// @desc to get the profile info of the current user
// @access private
router.get(
  "/",
  passport.authenticate("jwt", { session: false }),
...
router.post(
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      ...
      //Add to exp array , to add it at the begining, we are doing 'unshift'
  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id })
      .then(profile => {
        ...
        // SPlice out of the array (remove from the array)
